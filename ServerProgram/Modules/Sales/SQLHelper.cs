...
namespace DevExpress.Demos.SalesDBGenerator {
    public class SqlHelper<T, Command>
        ...
        public int Execute(T connection, DbTransaction trans, string selectQuery, params DbParameter[] pars) {
            using(var sql = CreateCommand(selectQuery, connection)) {
                if(trans != null) sql.Transaction = trans;
                sql.CommandTimeout = 5000;
                if(pars != null)
                    sql.Parameters.AddRange(pars);
        ...
        public int Execute(T connection, string selectQuery, params DbParameter[] pars) {
            return Execute(connection, null, selectQuery, pars);
        }
