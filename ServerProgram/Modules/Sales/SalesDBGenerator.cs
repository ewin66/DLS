...
namespace DevExpress.Demos.SalesDBGenerator {
    public class SalesGenerator : DevExpress.SalesDemo.Model.IDataGenerator {
        ...
        DateTime minRequiredDate = new DateTime(2012, 1, 1);
        int maxId = 0;
        ...
        void GenerateSale(OleDbConnection connection, OleDbTransaction transaction, DateTime date) {
            int id = GetId();
            ...
            SqlHelper<OleDbConnection, OleDbCommand> helper = new SqlHelper<OleDbConnection, OleDbCommand>();
            helper.Execute(connection, transaction, "insert into sales (id, units, cost_per_unit, discount, total_cost, sale_date, productId, RegionId, ChannelId, SectorId) " +
                                       "values (@id, @units, @cost_per_unit, @discount, @total_cost, @sale_date, @productId, @RegionId, @ChannelId, @SectorId)",
                                         new OleDbParameter("@id", id),
                                         new OleDbParameter("@units", units),
                                         new OleDbParameter("@cost_per_unit", productPrice),
                                         new OleDbParameter("@discount", discount),
                                         new OleDbParameter("@total_cost", totalPrice),
                                         new OleDbParameter("@sale_date", date),
                                         new OleDbParameter("@productId", product.Id),
                                         new OleDbParameter("@RegionId", region),
                                         new OleDbParameter("@ChannelId", channel),
                                         new OleDbParameter("@SectorId", sector));
        }
